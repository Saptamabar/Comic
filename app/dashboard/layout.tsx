...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#fdf6e3] flex flex-col lg:flex-row font-mono">
      
      {/* SIDEBAR DESKTOP */}
      <aside id="desktop-nav" className="hidden lg:flex flex-col w-80 bg-[#3e2723] border-r-4 border-[#3e2723] min-h-screen sticky top-0 h-screen z-40 text-white">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3 transition-all border-2 border-transparent rounded-lg group
                  ${active ? `${item.color} text-[#3e2723] border-[#3e2723] shadow-[4px_4px_0_#fff]` : "text-white hover:bg-white/10"}`}
              >
                <item.icon size={24} className={active ? "text-[#3e2723]" : "text-gray-400 group-hover:text-white"} />
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-[#3e2723] border-b-4 border-[#ffca28] flex items-center justify-between px-4 z-50 shadow-lg">
        <Link id="top-bar-mobile-badge"  href="/dashboard/badges" className="bg-[#ffca28] border-2 border-[#3e2723] p-1.5 shadow-[2px_2px_0_#fff] -rotate-3 active:scale-90 transition-all group">
          <Medal size={22} className="text-[#3e2723]" strokeWidth={3} />
          <span className="absolute -bottom-8 left-0 bg-[#3e2723] text-white font-comic text-[8px] px-2 py-1 opacity-0 group-hover:opacity-100 whitespace-nowrap">Lencana</span>
        </Link>

        <span className="font-bangers text-3xl text-[#ffca28] italic tracking-wider">HISTOPLAY</span>
        ...
        <div id="top-bar-mobile-profil"  className="flex items-center gap-3">
          <div className="relative" ref={dropdownRef}>
            <button onClick={() => setShowDropdown(!showDropdown)} className="flex items-center gap-1 active:scale-95 transition-all">
              ...
              <ChevronDown size={14} className={`text-[#ffca28] transition-transform ${showDropdown ? "rotate-180" : ""}`} />
            </button>
            <AnimatePresence>
              {showDropdown && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 mt-3 w-48 bg-[#fdf6e3] border-4 border-[#3e2723] shadow-[6px_6px_0_#3e2723] z-50 overflow-hidden">
                  <Link href="/dashboard/profile" onClick={() => setShowDropdown(false)} className="flex items-center gap-3 p-3 font-bangers text-lg uppercase text-[#3e2723] hover:bg-[#ffca28] border-b-2 border-[#3e2723]">
      ...
      {/* BOTTOM NAV MOBILE */}
      <nav id="mobile-nav" className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#fdf6e3] border-t-4 border-[#3e2723] flex justify-around items-end h-16 pb-2 px-1">
        {mobileNav.map((item) => {
          const active = pathname === item.href;
          if (item.special) {
            return (
              <Link key={item.label} href={item.href} className="relative -top-6 active:scale-95 transition-transform">
                <div className={`absolute inset-0 border-4 border-[#3e2723] -rotate-12 scale-125 -z-10 shadow-[4px_4px_0_#3e2723] ${active ? "bg-[#b71c1c]" : "bg-[#ffca28]"}`} />
                <div className="bg-[#fdf6e3] border-4 border-[#3e2723] p-2 mb-1"><item.icon size={22} className="text-[#3e2723]" strokeWidth={3} /></div>
                <span className="text-[8px] font-black uppercase italic bg-[#3e2723] text-[#fdf6e3] px-1 absolute -bottom-1 left-1/2 -translate-x-1/2 whitespace-nowrap">Quest</span>
              </Link>
            );
          }
          return (
            <Link key={item.label} href={item.href} className="flex-1 flex flex-col items-center justify-center h-full relative">
              {active && <motion.div layoutId="navActive" className="absolute inset-x-2 inset-y-1 bg-[#ffca28]/30 border-2 border-[#3e2723] -rotate-3 -z-10" />}
              <item.icon size={20} className={`transition-all ${active ? "text-[#3e2723] -rotate-6 scale-110" : "text-[#3e2723]/60"}`} strokeWidth={active ? 3 : 2} />
              <span className={`text-[9px] font-black uppercase mt-1 tracking-tighter ${active ? "text-[#3e2723] italic" : "text-[#3e2723]/60"}`}>{item.label}</span>
            </Link>
          );
        })}
      </nav>
